...
class StripeStub
  def success_charge(subscription, opts = {})
    opts[:paid] = true
    charge(subscription, 'invoice.payment.succeeded', opts)
  end

  def failed_charge(subscription, opts = {})
    opts[:paid] = false
    charge(subscription, 'invoice.payment.failed', opts)
  end

  def charge(subscription, event, opts = {})
    ...
    event_object.paid = opts[:paid] || true
    event_object.subtotal = opts[:subtotal] || 1000
    event_object.total = opts[:total] || 1000
    event_object.period_start = p_start
    event_object.period_end = p_end
    event_object.attempt_count = opts[:attempt_count] || 0
    event_object.date = (opts[:date] || Time.now).to_i
    event_object.customer = subscription.send(:customer_uid)
    stripe_event(event, OpenStruct.new(object: event_object))
  end
